...
let db = new sqlite3.Database('./database.db', ((data) => {
  if (data === null) {
    ...
    db.serialize(() => {
      ...
      db.run('CREATE TABLE IF NOT EXISTS `cell_meta` (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, row_id TEXT, col_id TEXT, meta TEXT)');
      db.run('CREATE TABLE IF NOT EXISTS `row_order` (id INTEGER, sort_order INTEGER)');
      db.run('CREATE TABLE IF NOT EXISTS `sizes` (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, col_id TEXT, row_id INTEGER, size INTEGER)');
      db.run('CREATE TABLE IF NOT EXISTS `merged_cells` (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, ' +
      ...
      db.run('CREATE UNIQUE INDEX IF NOT EXISTS USER_INDEX ON cell_meta (row_id, col_id)');
      db.run('CREATE UNIQUE INDEX IF NOT EXISTS ROW_INDEX on row_order (id)');
      db.run('CREATE UNIQUE INDEX IF NOT EXISTS MERGED_INDEX on merged_cells (cell_row_id, cell_col_id)');
      db.run('CREATE UNIQUE INDEX IF NOT EXISTS COLUMN_SIZE_INDEX on sizes (col_id)');
      db.run('CREATE UNIQUE INDEX IF NOT EXISTS ROW_SIZE_INDEX on sizes (row_id)');
    });
  }
  // initailize settings
  db.serialize(() => {
  ...
  function initRowOrder() {
    db.serialize(() => {
      db.all('SELECT * FROM `row_order` LIMIT 1', (err, rows) => {
        if (rows.length === 0) {
          db.all('SELECT * FROM `data`', (dataError, dataRows) => {
            let stmt = db.prepare('INSERT INTO `row_order` (`id`, `sort_order`) VALUES (?, ?)');
            for (let i = 0; i < dataRows.length; i++) {
              stmt.run(dataRows[i].id, i + 1);
            }
            stmt.finalize();
          });
        }
...
router.post('/cell', jsonParser, (req, res) => {
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    db.run('INSERT INTO `cell_meta` (\'row_id\', \'col_id\', \'meta\') VALUES (?, ?, ?)', data, (err) => {
      if (err) {
        let update = [];
        update.push(data[2]);
        update.push(data[0]);
        update.push(data[1]);
        db.run('UPDATE `cell_meta` SET meta=? WHERE row_id=? AND col_id=?', update, (cellMetaError) => {
          if (cellMetaError) { return console.error(cellMetaError.message); }
          return true;
        });
      }
    });
...
router.put('/row', jsonParser, (req, res) => {
  db.serialize(() => {
    ...
    stmt.run(function(error) {
      if (!error) {
        db.get('SELECT * from `data` where id= ?', this.lastID, (dataError, row) => {
...
router.post('/cell/meta', jsonParser, (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run(`INSERT INTO 'cell_meta' (row_id, col_id, meta) VALUES ('${cellMeta.row}', '${cellMeta.column}', '${JSON.stringify(tmp)}')`, (error) => {
      if (error) {
        db.all(`SELECT meta FROM 'cell_meta' WHERE col_id = '${cellMeta.column}' AND row_id = '${cellMeta.row}'`, (err, row) => {
          if (!err) {
